...
Layout.prototype.resizeElement = function(element, horizontal, width, height, p){
    ...
    if(element.stretch){
        ...
        if(horizontal){
            width = el.outerWidth(true)-sizeDec;
        } else {
            height = el.outerHeight(true)-sizeDec;
        }
    }
    var ch = element.children || [];
    var nonstretch = 0;
    ...
    for(var i = 0; i<ch.length; i++){
        ...
        if(element.horizontal){
            x += !this.isVisible(child)? 0: child.outerWidth(true);
        } else {
            y += !this.isVisible(child)? 0: child.outerHeight(true);
        }
    }
    return {
